...
void makeSystHist_HighPt(int nsel = 0, int whichDY = 3, TString theHistName = "Pt", bool doXSRatio = false){
  ...
  TString label="MM";
  if (nsel==1){label="EE";}
  TH1D* histPred     = (TH1D*)_fileDef->Get(Form("hDDilHigh%s%s",theHistName.Data(),label.Data()));
  TH1D* histPredStat = (TH1D*)histPred->Clone(Form("hDDilHigh%s%s_Stat",theHistName.Data(),label.Data()));
  TH1D* histPred_PDF = (TH1D*)_fileDef->Get(Form("hDDilHigh%s%s_PDF",theHistName.Data(),label.Data()));
  TH1D* histPred_QCD = (TH1D*)_fileDef->Get(Form("hDDilHigh%s%s_QCD",theHistName.Data(),label.Data()));
  ...
  for(int i=1; i<=histDef->GetNbinsX(); i++){
    ...
    if(doMakeUseFixedSystUnf == true) systVal[0] = 0;//theSystUnfVal;
