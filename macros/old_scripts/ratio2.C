...
void ratio2(int nsel = 0, int ReBin = 1, TString XTitle = "N_{jets}", TString units = "", TString plotName = "histo_nice.root", TString outputName = "njets",
  ...
  char sys_name[200];
  sprintf(sys_name,"../macros/sys_plots/histoUnfoldingSyst%s_nsel%d_dy3_rebin1_default.root",keyLabel0.Data(),me);
  TFile* uncertainty=new TFile(sys_name,"read");if(!uncertainty) {printf("File %s does not exist\n",sys_name); return;}
  TH1D* sys=(TH1D*)uncertainty->Get("sysonly");
  if(!sys){printf("sys not found");}
