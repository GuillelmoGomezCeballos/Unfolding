...
void helper_function(int me=0,int DY=3){
  ...
  /****** input data *******/
  TString path="../inputs/";
  TFile* file_alt = new TFile(Form("input/data_test_%d%d.root",me,DY), "read");  if(!file_alt) {printf("File does not exist\n"); return;} //argument 0,0,1
  ...
  for (int i=1;i<=reco_bin;i++){
    if(resonant->GetBinError(i)/resonant->GetBinContent(i)<0.2){
      sigma_pdf[i-1]=1.0*(pdf->GetBinContent(i)-resonant->GetBinContent(i))/resonant->GetBinContent(i);
      sigma_qcd[i-1]=1.0*(qcd->GetBinContent(i)-resonant->GetBinContent(i))/resonant->GetBinContent(i);
    }
    else{
      sigma_pdf[i-1]=0.0;
      sigma_qcd[i-1]=0.0;
  ...
  for (int i=1;i<=gen_bin;i++){
    ...
    for (int j=1;j<=mom;j++){
      ...
      for(int k=1;k<=reco_bin;k++){
        ...
        sigma_momres[k-1]=1.0*sigma_lepeff[k-1]/normal->GetBinContent(k);
  ...
  for (int i=1;i<=mom;i++){
    ...
    for(int k=1;k<=reco_bin;k++){
      sigma_momres[k-1]=1.0*temp->GetBinContent(k)-normal->GetBinContent(k);
      sigma_momres[k-1]=1.0*sigma_momres[k-1]/normal->GetBinContent(k);
      if(normal->GetBinError(k)/normal->GetBinContent(k)>0.2){sigma_momres[k-1]=0.0;}
    }
    for(int k=1;k<=reco_bin;k++){
      for(int q=1;q<=reco_bin;q++){
  ...
  for (int i=1;i<=gen_bin;i++){
	...
	sigma_stat[k-1]=1.0*sigma_stat[k-1]/normal->GetBinContent(k);
	if(normal->GetBinError(k)/normal->GetBinContent(k)>0.2){sigma_stat[k-1]=0.0;}
	//if (sigma_stat[k-1]<=min){sigma_stat[k-1]=0.0;}
      }
      for(int k=1;k<=reco_bin;k++){
	for(int q=1;q<=reco_bin;q++){
  ...
  /*for (int i=1;i<=reco_bin;i++){
    int bx=covariance_all->GetXaxis()->GetBinWidth(i);
    for (int j=1;j<=reco_bin;j++){
      int by=covariance_all->GetYaxis()->GetBinWidth(j);
      covariance_all->SetBinContent(i,j,covariance_all->GetBinContent(i,j)/(bx*by));
    }
    }*/
  ...
  covariance_all->Draw("colz");
  c1->SetLogz();
  gStyle->SetOptStat(0);
  covariance_all->SetTitle("");
  covariance_all->GetXaxis()->SetTitle("Reconstructed P_{T} (GeV)");
  covariance_all->GetXaxis()->SetLabelFont  (   42);
  covariance_all->GetXaxis()->SetLabelOffset(0.015);
  covariance_all->GetXaxis()->SetLabelSize  (0.030);
  covariance_all->GetXaxis()->SetNdivisions (  505);
  covariance_all->GetXaxis()->SetTitleFont  (   42);
  covariance_all->GetXaxis()->SetTitleOffset( 1.0);
  covariance_all->GetXaxis()->SetTitleSize  (0.040);
  covariance_all->GetXaxis()->SetTickLength (0.07 );

  covariance_all->GetYaxis()->SetTitle("Reconstructed P_{T} (GeV)");
  covariance_all->GetYaxis()->SetLabelFont  (   42);
  covariance_all->GetYaxis()->SetLabelOffset(0.015);
  covariance_all->GetYaxis()->SetLabelSize  (0.030);
  covariance_all->GetYaxis()->SetNdivisions (  505);
  covariance_all->GetYaxis()->SetTitleFont  (   42);
  covariance_all->GetYaxis()->SetTitleOffset(  1.2);
  covariance_all->GetYaxis()->SetTitleSize  (0.040);
  covariance_all->GetYaxis()->SetTickLength (0.03 );
  // covariance_all->GetZaxis()->SetTitle("(Relative Uncertainty)^{2}");
  //covariance_all->GetZaxis()->SetTitleOffset(0.50);
