...
void helper_function(int nsel=0,int whichDY=0, int rebin=1, TString theHistName = "Pt", TString suffix = "", int suffixStat = -1){
  ...
  if     (whichDY == 0){
    ...
    _file1 = TFile::Open("/afs/cern.ch/work/c/ceballos/public/samples/panda/v_005_0/genZpt_LO.root");
    ...
    _file0 = TFile::Open(Form("%s/histoDY%dzll%sRecGen.root",theInputFolder.Data(),whichDY,theHistName.Data()));
    _file1 = TFile::Open("/afs/cern.ch/work/c/ceballos/public/samples/panda/v_005_0/genZpt_NLO.root");
    _file2 = TFile::Open(Form("%s/histoDY%dzll%sRecGen.root",theInputFolder.Data(),2,theHistName.Data()));
  }
