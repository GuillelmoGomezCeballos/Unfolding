...
#include <iostream>
#include <fstream>
void helper_function(int me, int DY, TString theHistName, int count, TString path = "outputs_update/", int reBin = 1){

  char filename[300];
  //sprintf(filename,"%s/histoUnfolding_XSRatioSyst%s_nsel%d_dy%d_rebin%d_default.root",path.Data(),theHistName.Data(),me,DY,reBin);
  ...
  //TH2D* covariance_totsum     = new TH2D(Form("d%02d-x01-y01",count),     Form("covariance_XSRatio_%s_nsel%d_dy%d",theHistName.Data(),me,DY),nBinGen, xbinsGen, nBinGen, xbinsGen);
  TH2D* covariance_totsum     = new TH2D(Form("d%02d-x01-y01",count),     Form("covariance_%s_nsel%d_dy%d",theHistName.Data(),me,DY),nBinGen, xbinsGen, nBinGen, xbinsGen);

  TH1D* unfold = (TH1D*)_file0->Get("unfold");
  unfold->SetName(Form("d%02d-x01-y01",count+1));
  //unfold->SetTitle(Form("unfold_XSRatio_%s_nsel%d_dy%d", theHistName.Data(),me,DY));
  unfold->SetTitle(Form("unfold_%s_nsel%d_dy%d", theHistName.Data(),me,DY));
  ...
  unfold->Scale(1.0/35900.0);

  char outputLimits[200];
  sprintf(outputLimits,"forRIVET/matrix%02d.root",count);
...
void matrixForRIVET(){
  ...
  for (int x=0; x<8; x++) {
    for(int i=0;i<=1;i++){
      for(int j=3;j<=3;j++){
	helper_function(i,j,histNames[x].Data(), gc);
	gc+=2;
      }
    }
  }
